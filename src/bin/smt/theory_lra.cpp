...
namespace smt {
    ...
    class theory_lra::imp {        
        ...
        void init_solver() {
            ...
            for (int i = 0; i<5; i++){
            	linearBounds.push_back(linearBoundArray[i]);
            }
        }
        ...
        */
        ...
        void addTangentConstraint(double aVal, double val,int inValIndex, int outValIndex){
			...
			if (!ctx().b_internalized(finalFormula)){
				std::cout << "ASSERTING: tangent: finalConstraint " << mk_pp(finalFormula, m) << "\n";
				ctx().internalize_assertion(finalFormula, 0, 1);
				/*bool_var bv = ctx().mk_bool_var(finalFormula);
				theory_var v = mk_var(leftSide);
				lp::bound* newBound = alloc(lp::bound, bv, v, r, bKind);
				ctx().set_var_theory(bv, get_id());

				mk_bound_axioms(*newBound);
				updt_unassigned_bounds(v, +1);
				m_bounds[v].push_back(newBound);
				m_bounds_trail.push_back(v);
				m_bool_var2bound.insert(bv, newBound);
				assign_eh(bv, true);
				miniPropagate();*/
			}
			//pop_scope_eh(1);
        ...
        void ignoreCurrentSolution(Eigen::MatrixXd hyperRectangle, int* inValIndices){
        	...
        	app* theFinalFormula = mk_or(m,numVars,allFormulas);

        	if (!ctx().b_internalized(theFinalFormula)){
				std::cout << "ASSERTING: ignoring solution: " << mk_pp(theFinalFormula,m) << "\n";
				//ctx().reset_assumptions();
				ctx().internalize_assertion(theFinalFormula,0,1);
				//ignoringSolution = true;
			}
        ...
        void propagate() {
			...
			//std::cout << "oldScopeSize "<<oldScopeSize << "\n";
			//std::cout << "m_scopes.size() "<< m_scopes.size() << "\n";
			//std::cout << "oldVarTrailSize "<<oldVarTrailSize << "\n";
			//std::cout << "m_var_trail_size() "<<m_var_trail.size() << "\n";
            ...
            case l_true:
            	...
            	if(m_asserted_atoms.size() >= numOrigConstraints){ //need to figure out a better way to check if this is a complete assignment
            		...
            		if (numVars < 4){
						...
						if(m_stats.m_make_feasible%2 == 0){
							for(int ii = 0; ii<numVars; ii++){
