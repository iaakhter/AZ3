...
#define SMALL_NUM_PARENTS 3
    ...
    */
    bool context::propagate_atoms() {
    	SASSERT(!inconsistent());
        //std::cout << "smt_context: propagate_atoms size of m_atom_propagation_queue " << m_atom_propagation_queue.size() << "\n";
        for (unsigned i = 0; i < m_atom_propagation_queue.size(); i++) {
        	//std::cout << "smt_context::atom # in propagation queue: " << i << "\n";
            SASSERT(!inconsistent());
            literal  l = m_atom_propagation_queue[i];
            bool_var v = l.var();
            bool_var_data & d = get_bdata(v);
            lbool val  = get_assignment(v);
            //std::cout << "smt_context:: propagate_atoms propagating atom " << mk_pp(bool_var2expr(v), m_manager) <<  "\n";
            ...
            if (d.is_eq()) {
            	...
            	//std::cout << "propagate_atoms propagating atom " << mk_pp(bool_var2expr(v), m_manager) <<  "\n";
    ...
    */
    ...
    void context::relevant_eh(expr * n) {
    	//std::cout << "smt_context::relevant_eh " << mk_pp(n, m_manager) <<  "\n";
        if (b_internalized(n)) {
            ...
            if (d.is_atom() && (d.is_quantifier() || m_fparams.m_relevancy_lvl >= 2)) {
            	//std::cout << "smt_context::relevant_eh pushedToPropagationQueeu " << mk_pp(n, m_manager) <<  "\n";
                lbool val  = get_assignment(v);
                if (val != l_undef){
        ...
        //std::cout << "size of m_atom_propagation_queue after ctx()::relevant_eh " << m_atom_propagation_queue.size() << "\n";
    }
    ...
    */
    void context::propagate_relevancy(unsigned qhead) {
        ...
        while (qhead < sz) {
            ...
            //std::cout << "ctx()::propagate_relevancy expr " << mk_pp(n,m_manager) << "\n";
            m_relevancy_propagator->assign_eh(n, !l.sign());
            //std::cout << "ctx()::propagate_relevancy m_atom_propagation_queue.size() " << m_atom_propagation_queue.size() << "\n";
        }
        m_relevancy_propagator->propagate();
    }

    bool context::propagate_theories() {
    	std::cout << "Propagating theories\n";
    ...
    int context::getNumRemainingCaseSplits(){
    	return m_case_split_queue->queueSize();
    }

    bool context::propagate() {
        TRACE("propagate", tout << "propagating... " << m_qhead << ":" << m_assigned_literals.size() << "\n";);
        //std::cout << "propagate propagating... " << m_qhead << ":" << m_assigned_literals.size() << "\n";
        //std::cout << "Display m_case_split_queue\n";
        //std::cout << "Size of queue "<< getNumRemainingCaseSplits() << "\n";
		//m_case_split_queue->display(std::cout);
        //std::cout << m_b_internalized_stack.size() << "\n";
        //std::cout << m_e_internalized_stack.size() << "\n";
        //std::cout << "m_scope_lvl " << 	m_scope_lvl << "\n";
		//std::cout << "\n";
        //std::cout << "m_num_mk_bool_var " << m_stats.m_num_mk_bool_var << "\n";
        //std::cout << "numRestarts " << m_stats.m_num_restarts++ << "\n";
        while (true) {
        	//std::cout << "numFormulas in ctx().propagate() " << m_asserted_formulas.get_num_formulas() << "\n";
            if (inconsistent())
                return false;
            unsigned qhead = m_qhead;
           // std::cout << "qhead " << qhead << " m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            if (!bcp())
                return false;
            //std::cout << "####1\n";
			...
			//std::cout << "###propagate propagating... " << m_qhead << ":" << m_assigned_literals.size() << "\n";
            SASSERT(!inconsistent());
            //std::cout << "check001 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            propagate_relevancy(qhead);
            //std::cout << "check01 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check01 final_check_status " << final_check() << "\n";
            //std::cout << "check01 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check01 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check01 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check01 ctx::propagate() qhead " << qhead << "\n";
            if (inconsistent()) 
                return false;
            //std::cout << "check02 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check02 final_check_status " << final_check() << "\n";
            //std::cout << "check02 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check02 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check02 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check02 ctx::propagate() qhead " << qhead << "\n";
            if (!propagate_atoms())
                return false;
            //std::cout << "check03 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check03 final_check_status " << final_check() << "\n";
            //std::cout << "check03 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check03 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check03 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check03 ctx::propagate() qhead " << qhead << "\n";
            if (!propagate_eqs())
                return false;
            //std::cout << "check04 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check04 final_check_status " << final_check() << "\n";
            //std::cout << "check04 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check04 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check04 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check04 ctx::propagate() qhead " << qhead << "\n";
            propagate_th_eqs();
            //std::cout << "check05 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check05 final_check_status " << final_check() << "\n";
            //std::cout << "check05 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check05 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check05 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check05 ctx::propagate() qhead " << qhead << "\n";
            propagate_th_diseqs();
            //std::cout << "check06 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check06 final_check_status " << final_check() << "\n";
            //std::cout << "check06 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check06 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check06 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check06 ctx::propagate() qhead " << qhead << "\n";
            if (inconsistent()) 
                return false;
            //std::cout << "check07 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check07 final_check_status " << final_check() << "\n";
            //std::cout << "check07 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check07 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check07 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check07 ctx::propagate() qhead " << qhead << "\n";
            //std::cout << "check07 can_propagate " << !can_propagate() << "\n";

            if (!propagate_theories())
                return false;
            //std::cout << "check1 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check1 final_check_status " << final_check() << "\n";
            //std::cout << "check1 can_propagate() " << can_propagate() << "\n";
            //std::cout << "check1 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check1 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check1 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check1 ctx::propagate() qhead " << qhead << "\n";
            m_qmanager->propagate();
            //std::cout << "check2 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check2 final_check_status " << final_check() << "\n";
            //std::cout << "check2 can_propagate() " << can_propagate() << "\n";
            //std::cout << "check2 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check2 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check2 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check2 ctx::propagate() qhead " << qhead << "\n";
            if (inconsistent())
                return false;
            //std::cout << "check3 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "check3 final_check_status " << final_check() << "\n";
            //std::cout << "check3 can_propagate() " << can_propagate() << "\n";
            //std::cout << "check3 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check3 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check3 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check3 ctx::propagate() qhead " << qhead << "\n";
            if (resource_limits_exceeded()) {
                m_qhead = qhead;
                return true;
            }
            std::cout << "check4 ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "***propagate propagating... " << m_qhead << ":" << m_assigned_literals.size() << "\n";
            //std::cout << "check4 final_check_status " << final_check() << "\n";
            //std::cout << "check4 can_propagate() " << can_propagate() << "\n";
            //std::cout << "check4 m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "check4 internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "check4 ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "check4 ctx::propagate() qhead " << qhead << "\n";
            std::cout<<"\n";
           // std::cout << "***********Size of queue "<< getNumRemainingCaseSplits() << "\n";
            if (!can_propagate()) {
            ...
            //std::cout << "size of m_atom_propagation_queue after a cycle of while loop ctx::propagate() " << m_atom_propagation_queue.size() << "\n";
            //std::cout << "after a cycle final_check_status " << final_check() << "\n";
            //std::cout << "after a cycle can_propagate() " << can_propagate() << "\n";
            //std::cout << "after a cycle m_watches.size() "<<m_watches.size() << "\n";
            //std::cout << "after a cycle internalizedStackSize " << m_b_internalized_stack.size() << "\n";
            //std::cout << "after a cycle ctx::propagate() m_assigned_literals.size() " << m_assigned_literals.size() << "\n";
            //std::cout << "after a cycle of while loop ctx::propagate() qhead " << qhead << "\n";
            //m_asserted_formulas.display(std::cout);
            //std::cout << "size m_asserted_formulas after a cycle of while loop ctx::propagate() " << m_asserted_formulas.get_num_formulas() << "\n";
        }
    }
    ...
    */
    bool context::decide() {
        ...
        if (var == null_bool_var) {
        	std::cout << "NULLBOOLVAR IN DECIDE()\n";
            return false;
        }
    ...
    */
    ...
    void context::internalize_assertions() {
        ...
        reduce_assertions();
        //std::cout << "m_asserted_formulas_display\n";
        //m_asserted_formulas.display(std::cout);
        //std::cout << "size m_asserted_formulas " << m_asserted_formulas.get_num_formulas() << "\n";
        if (!m_asserted_formulas.inconsistent()) {
            unsigned sz    = m_asserted_formulas.get_num_formulas();
            unsigned qhead = m_asserted_formulas.get_qhead();
            while (qhead < sz) {
            	//std::cout << "ctx()::internalize_assertions " << "qhead " << qhead << " sz " << sz << "\n";
                if (get_cancel_flag()) {
                    m_asserted_formulas.commit(qhead);
                    return;
                }
                expr * f   = m_asserted_formulas.get_formula(qhead);
                //std::cout << "expr " << mk_pp(f, m_manager) << "\n";
                proof * pr = m_asserted_formulas.get_formula_proof(qhead);
                /*std::cout << "smt_context:: INTERNALIZE ASSERTIONS expr\n";
                std::cout << mk_pp(f,m_manager);
                std::cout << "\n";
                std::cout << "smt_context::INTERNALIZE_ASSERTIONS proof\n";
                std::cout << mk_ll_pp(pr, m_manager);
                std::cout << "\n";*/
        ...
        //std::cout << "size of m_atom_propagation_queue after ctx()::internalize_assertions " <<
         //       		m_atom_propagation_queue.size()<<"\n";
    }
    ...
    */
    ...
    lbool context::check(unsigned ext_num_assumptions, expr * const * ext_assumptions, bool reset_cancel, bool already_did_theory_assumptions) {
        ...
        TRACE("check_bug", tout << "STARTING check(num_assumptions, assumptions)\n";
              ...
              tout << "-----------------------\n";
              display(tout););
        //std::cout << "numFormulas in ctx().check() " << m_asserted_formulas.get_num_formulas() << "\n";
        ...
        if (inconsistent()) {
            ...
            setup_context(false);
            internalize_assertions();
           // std::cout << "size of m_atom_propagation_queue after ctx()::check ln 3415 " <<
            //                                               		m_atom_propagation_queue.size()<<"\n";
            TRACE("after_internalize_assertions", display(tout););
            if (m_asserted_formulas.inconsistent()) {
                r = l_false;
                //std::cout << "size of m_atom_propagation_queue after ctx()::check ln 3420 " <<
                //                                                           		m_atom_propagation_queue.size()<<"\n";
            }
            else {
                init_assumptions(num_assumptions, assumptions);
                //std::cout << "size of m_atom_propagation_queue after ctx()::check ln 3425 " <<
                //                                                           		m_atom_propagation_queue.size()<<"\n";
                TRACE("after_internalization", display(tout););
                if (inconsistent()) {
                	//std::cout << "size of m_atom_propagation_queue after ctx()::check ln 3429 " <<
					//					 m_atom_propagation_queue.size()<<"\n";
                    VERIFY(!resolve_conflict()); // build the proof
                    //std::cout << "size of m_atom_propagation_queue after ctx()::check ln 3432 " <<
					//					 m_atom_propagation_queue.size()<<"\n";
                    ...
                    //std::cout << "size of m_atom_propagation_queue after ctx()::check ln 3436 " <<
                    //                                         m_atom_propagation_queue.size()<<"\n";
                }
                else {
                    r = search();
    ...
    lbool context::search() {
        ...
        /*std::cout << "search " << "\n";
        display(std::cout);
        display_enodes_lbls(std::cout);
        std::cout << "search_detail\n";
        m_asserted_formulas.display(std::cout);
        std::cout << "\n";*/
        init_search();
        flet<bool> l(m_searching, true);
        TRACE("after_init_search", display(tout););
        /*std::cout << "after_init_search\n";
        display(std::cout);
        std::cout << "\n";*/
    ...
    lbool context::bounded_search() {
        ...
        while (true) {
			...
			m_case_split_queue->next_case_split(var, phase);*/
            /*std::cout << "Display m_case_split_queue\n";
            std::cout << m_case_split_queue->m_queue.size() << "\n";
            m_case_split_queue->display(std::cout);
            std::cout << "\n";*/
			/*if (var == null_bool_var) {
				std::cout << "NULLBOOLVAR LN::3816\n";
			}*/
            if (!decide()) {
            	std::cout << "DECIDING\n";
    ...
    bool context::resolve_conflict() {
        ...
        if (m_conflict_resolution->resolve(m_conflict, m_not_l)) {
            ...
            /*std::cout << "resolve_conflict_bug m_scope_lvl: " << m_scope_lvl << ", new_lvl: " << new_lvl<< ", lemma_intern_lvl: " << m_conflict_resolution->get_lemma_intern_lvl() << "\n";
            std::cout << "num_lits: " << num_lits << "\n";
            for (unsigned i = 0; i < num_lits; i++) {
            	literal l = lits[i];
            	tout << l << " ";
            	display_literal(tout, l);
            	std::cout << ", ilvl: " << get_intern_level(l.var()) << "\n"
				   << mk_pp(bool_var2expr(l.var()), m_manager) << "\n";
            }*/
            if (m_manager.has_trace_stream()) {
            ...
            TRACE("context_lemma", tout << "new lemma: "; 
                  ...
                  tout << "\n";);
            /*std::cout << "context_lemma new lemma: ";
            literal_vector v(num_lits, lits);
            std::sort(v.begin(), v.end());
		  for (unsigned i = 0; i < num_lits; i++) {
			  display_literal(std::cout, v[i]);
			  std::cout << "\n";
			  v[i].display(std::cout, m_manager, m_bool_var2expr.c_ptr());
			  std::cout << "\n\n";
		  }
		  std::cout << "\n";*/
            decay_bvar_activity();
