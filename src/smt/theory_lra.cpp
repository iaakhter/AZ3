...
namespace smt {
    ...
    class theory_lra::imp {        
        ...
        public:
            ...
            expr_ref_vector&     terms() {
            	ref_vector<expr, ast_manager>::element_ref theTerm = m_st.m_terms[0];
            	return m_st.m_terms;
            }
            ...
            void push(expr* e, rational c) {
            	m_st.m_terms.push_back(e);
            	m_st.m_coeffs.push_back(c); }
        ...
        //Itrat Solver global variables
        //std::vector<double> linearBounds;
        std::list<Eigen::MatrixXd> allUniqueHyperRectangles;
        int countAdditionalConstraints = 0;
        double distance = 0.2;
        std::vector<std::vector<double>> linearBounds;
        ...
        void init_solver() {
            ...
            double linearBoundArray[5] = {-4.0,-0.5,0.0,0.5,3.5};
            std::vector<double> linearBoundVector(linearBoundArray, linearBoundArray + sizeof(linearBoundArray)/sizeof(double));
            linearBounds.push_back(linearBoundVector);
            linearBounds.push_back(linearBoundVector);
            linearBounds.push_back(linearBoundVector);
            linearBounds.push_back(linearBoundVector);
        }
        ...
        void linearize(scoped_internalize_state& st) { 
            ...
            while (index < terms.size()) {
                ...
                ref_vector<expr, ast_manager>::element_ref nExpr = terms[index];
                st.terms_to_internalize().push_back(n);
                if (a.is_add(n)) {
                    unsigned sz = to_app(n)->get_num_args();
                    for (unsigned i = 0; i < sz; ++i) {
                    	rational theCoeff = coeffs[index];
                    	expr argument = *to_app(n)->get_arg(i);
        ...
        theory_var mk_var(expr* n, bool internalize = true) {
            ...
            /*std::cout << "theory_var " << v << " mk_var " << mk_pp(n, m) << "\n";
            std::cout << "decl_kind " <<to_app(n)->get_decl()->get_name() << "\n\n";
            std::cout << "num_args " << to_app(n)->get_num_args() << "\n";
            int numArgs = to_app(n)->get_num_args();
            for(int i = 0; i<numArgs; i++){
            	std::cout << "arg# " << i << " "<<mk_pp(to_app(n)->get_arg(i),m) << "\n";
            }*/
            return v;
        }
        ...
        void add_ineq_constraint(lean::constraint_index index, literal lit) {
        	int sizeOfConstraints = m_constraint_sources.size();
            ...
            //std::cout << "Inequality constraint in theory_lra::add_ineq_constraint\n";
            m_solver->print_constraint(index,std::cout);
        }
        ...
        theory_var internalize_def(app* term) {
        	//std::cout << "Internalized def" << mk_pp(term, m) << "\n";
            scoped_internalize_state st(*this);
            linearize_term(term, st);
            if (is_unit_var(st)) {
                return st.vars()[0];
                ...
                if (vi == UINT_MAX) {
                	//std::__1::pair<rational, unsigned int> leftSide = m_left_side[0];
    ...
    public:
        ...
        bool internalize_atom_strict(app * atom, bool gate_ctx) {
            ...
            //std::cout << "in internalized_atom_strict::ln 721\n";
            //std::cout << "Internalized " << mk_pp(atom, m) << "\n";
            mk_bound_axioms(*b);
            //add_use_lists(b);
            return true;
        }
        ...
        void assign_eh(bool_var v, bool is_true) {
            ...
            //std::cout << "in assign_eh " << mk_pp(ctx().bool_var2expr(v), m) << " " << (is_true?"true":"false") << "\n";
            //std::cout << "\n";
        ...
        */
        ...
        double tanhFun(double aVal, double val){
        	//std::cout << "aVal in tanhFun " << aVal << "\n";
        	return tanh(aVal*val);
        }

        double tanhFunder(double aVal, double val){
        	double den = cosh(aVal*val)*cosh(aVal*val);
        	return aVal/den;
        }

        void addTangentConstraint(double aVal, double val,int inValIndex, int outValIndex){
        	std::cout << "adding tangent constraint\n";
        	std::cout << "val " << val << "\n";
        	double tanhVal = tanhFun(aVal,val);
        	double tanhDer = tanhFunder(aVal,val);
        	double cVal = tanhVal - tanhDer*val;
        	theory_var inValTheoryVar = m_var_index2theory_var[inValIndex];
        	theory_var outValTheoryVar = m_var_index2theory_var[outValIndex];

			expr* inVal = get_enode(inValTheoryVar)->get_owner();
			expr* outVal = get_enode(outValTheoryVar)->get_owner();

			//push_scope_eh();
			char derString[64];
			snprintf(derString, sizeof(derString), "%f", tanhDer);
			app* derEqn = a.mk_real(rational(derString));
			std::cout << "tangent: derEqn " << mk_pp(derEqn, m) << "\n";

			char cString[64];
			snprintf(cString, sizeof(cString), "%f", cVal);
			app* rightSide = a.mk_real(rational(cString));
			std::cout << "tangent: cEqn " << mk_pp(rightSide, m) << "\n";

			app* leftSide = a.mk_sub(outVal, a.mk_mul(derEqn,inVal));

			char zeroValString[64];
			snprintf(zeroValString,sizeof(zeroValString),"%f",0.0);
			app* zeroValEqn = a.mk_real(rational(zeroValString));

			app* formula;
			app* impliesCondition;
			if((aVal <= 0 && val <= 0) || (aVal >=0 && val >=0)){
				formula = a.mk_le(leftSide, rightSide);
				if(aVal <= 0){
					impliesCondition = m.mk_not(a.mk_le(inVal,zeroValEqn));
				}
				else{
					impliesCondition = m.mk_not(a.mk_ge(inVal,zeroValEqn));
				}
			}
			else if((aVal <= 0 && val >=0) || (aVal >= 0 && val <= 0)){
				formula = a.mk_ge(leftSide, rightSide);
				if(aVal <= 0){
					impliesCondition = m.mk_not(a.mk_ge(inVal,zeroValEqn));
				}
				else{
			...
			app* finalFormula = m.mk_or(impliesCondition,formula);

			/*literal_buffer lits;
			unsigned num = to_app(finalFormula)->get_num_args();
			std::cout << "numExpr " << num << "\n";
			for (unsigned i = 0; i < num; i++) {
				expr * arg = to_app(finalFormula)->get_arg(i);
				ctx().internalize(arg, true);
				lits.push_back(ctx().get_literal(arg));
			}*/

			if (!ctx().b_internalized(finalFormula)){
				std::cout << "ASSERTING: finalFormula: " << mk_pp(finalFormula,m) << "\n";
				ctx().internalize_assertion(finalFormula, 0, 1);
				//ctx().mk_clause(lits.size(), lits.c_ptr(),0,CLS_AUX_LEMMA);
				//ctx().mark_as_relevant(finalFormula);
			}
        }

        /*void addSecantConstraint(double aVal, double val, int inValIndex, int outValIndex){
        	std::cout << "adding secant constraint\n";
        	double lowVal;
        	double highVal;

        	for (int i = 0; i<linearBounds.size()-1; i++){
        		if (val >= linearBounds[i] && val <= linearBounds[i+1]){
        			lowVal = linearBounds[i];
        			highVal = linearBounds[i+1];
        			break;
        		}
        	}

        	double lowValOut = tanhFun(aVal,lowVal);
        	double highValOut = tanhFun(aVal,highVal);
        	double valOut = tanhFun(aVal,val);

        	double der1 = (highValOut - lowValOut)/(highVal - lowVal);
        	double c1 = lowValOut - der1*lowVal;

        	double der2 = (highValOut - valOut)/(highVal - val);
        	double c2 = highValOut - der2*highVal;

        	std::cout << "adding secant constraint\n";
        	std::cout << "val: "<< val << " valOut: " << valOut
        			<< " lowVal: "<<lowVal<<" lowValOut: " << lowValOut
					<< " highVal: "<<highVal<<" highValOut: " << highValOut
        			<< "der1: " << der1 << " der2: " << der2 << "\n"
					<< " c1: " << c1 << " c2: " << c2 << "\n";
			...
			char derString1[64];
			snprintf(derString1, sizeof(derString1), "%f", der1);
			app* derEqn1 = a.mk_real(rational(derString1));
			char cString1[64];
			snprintf(cString1, sizeof(cString1), "%f", c1);
			app* leftSide1 = a.mk_sub(outVal, a.mk_mul(derEqn1,inVal));
			rational r1 = rational(cString1);
			app* rightSide1 = a.mk_real(r1);
			app* formula1;
			if((aVal <= 0 && val <= 0)||(aVal >= 0 && val >= 0)){
				formula1 = a.mk_ge(leftSide1, rightSide1);
			}
			else if((aVal <= 0 && val >= 0)||(aVal >= 0 && val <= 0)){
				formula1 = a.mk_le(leftSide1, rightSide1);
			}
			std::cout << "secant: formula1 " << mk_pp(formula1, m) << "\n";

			char derString2[64];
			snprintf(derString2, sizeof(derString2), "%f", der2);
			app* derEqn2 = a.mk_real(rational(derString2));
			char cString2[64];
			snprintf(cString2, sizeof(cString2), "%f", c2);
			app* leftSide2 = a.mk_sub(outVal, a.mk_mul(derEqn2,inVal));
			rational r2 = rational(cString2);
			app* rightSide2 = a.mk_real(r2);
			app* formula2;
			if((aVal <= 0 && val <= 0)||(aVal >= 0 && val >= 0)){
				formula2 = a.mk_ge(leftSide2, rightSide2);
			}
			else if((aVal <= 0 && val >= 0)||(aVal >= 0 && val <= 0)){
				formula2 = a.mk_le(leftSide2, rightSide2);
			}
			std::cout << "secant: formula2 " << mk_pp(formula2, m) << "\n";

			char lowValString[64];
			snprintf(lowValString,sizeof(lowValString),"%f",lowVal);
			app* lowValEqn = a.mk_real(rational(lowValString));
			app* cond1 = a.mk_ge(inVal,lowValEqn);
			char highValString[64];
			snprintf(highValString,sizeof(highValString),"%f",highVal);
			app* highValEqn = a.mk_real(rational(highValString));
			app* cond2 = a.mk_le(inVal,highValEqn);
			app *impliesCondition = m.mk_not(m.mk_and(cond1,cond2));

			app* implicationRhs = m.mk_or(formula1,formula2);
			app* finalFormula = m.mk_or(impliesCondition,implicationRhs);
			lp::bound_kind bKind = lp::bound_kind::lower_t;
			if (!ctx().b_internalized(finalFormula) && val >= lowVal && val <= highVal ){
				std::cout << "ASSERTING: finalFormula: " << mk_pp(finalFormula,m) << "\n";
				ctx().internalize_assertion(finalFormula, 0, 1);
        ...
        void addTriangleConstraint(double aVal,int inValIndex, int outValIndex, double lowVal, double highVal){
			std::cout << "adding triangle constraint\n";
			addTangentConstraint(aVal,lowVal,inValIndex,outValIndex);
			addTangentConstraint(aVal,highVal,inValIndex,outValIndex);
			double lowValOut = tanhFun(aVal,lowVal);
			double highValOut = tanhFun(aVal,highVal);

			double der = (highValOut - lowValOut)/(highVal - lowVal);
			double c = lowValOut - der*lowVal;

			std::cout << "adding secant constraint\n";
			std::cout << " lowVal: "<<lowVal<<" lowValOut: " << lowValOut
					<< " highVal: "<<highVal<<" highValOut: " << highValOut
					<< " der: " << der;
			...
			char derString[64];
			snprintf(derString, sizeof(derString), "%f", der);
			app* derEqn = a.mk_real(rational(derString));
			char cString[64];
			snprintf(cString, sizeof(cString), "%f", c);
			app* leftSide = a.mk_sub(outVal, a.mk_mul(derEqn,inVal));
			rational r = rational(cString);
			app* rightSide = a.mk_real(r);
			app* formula;
			if((aVal <= 0 && lowVal <= 0 && highVal <= 0) || (aVal >= 0 && lowVal >= 0 && highVal >= 0)){
				formula = a.mk_ge(leftSide, rightSide);
			}
			else if((aVal <= 0 && lowVal >= 0 && highVal >= 0) || (aVal >= 0 && lowVal <= 0 && highVal <= 0)){
				formula = a.mk_le(leftSide, rightSide);
			}
			std::cout << "secant: formula " << mk_pp(formula, m) << "\n";
        ...
        void addTriangleConstraints(double aForTanh, int* inValIndices, int* outValIndices, int* voltIndices, Eigen::MatrixXd hyperRectangle){
        	for(int ii = 0; ii< hyperRectangle.rows(); ii++){
        		addTriangleConstraint(aForTanh,inValIndices[ii],outValIndices[ii],hyperRectangle(voltIndices[ii],0),hyperRectangle(voltIndices[ii],1));
        	}
        }

        /*void constructNewConstraint(double aForTanh, double* assign1Floats, double* assign2Floats, int* inValIndices, int* outValIndices, int numVars){
        	for(int ii = 0; ii<numVars; ii++){
        		double assign1Float = assign1Floats[ii];
        		double assign2Float = assign2Floats[ii];
        		int inValIndex = inValIndices[ii];
        		int outValIndex = outValIndices[ii];
				double tanhOrig = tanhFun(aForTanh, assign1Float);
				if (aForTanh >= 0){
					if (assign1Float >= 0){
						if (assign2Float >= tanhOrig){
							addTangentConstraint(aForTanh, assign1Float,inValIndex,outValIndex);
						}
						else{
							addSecantConstraint(aForTanh, assign1Float,inValIndex,outValIndex);
						}
					}
					else{
						if (assign2Float <= tanhOrig){
        ...
        Eigen::MatrixXd fun1Num(Eigen::MatrixXd x,double aVal,double params[2]){
        	Eigen::MatrixXd result(x.rows(), 1);
        	for (int i = 0; i<x.rows(); i++){
        		result(i,0) = tanhFun(aVal,x(i,0)) - params[0]*x(i,0) - params[1];
        	}
        	return result;
        }

        Eigen::MatrixXd fun2Num(Eigen::MatrixXd x,double aVal,double params[2]){
			Eigen::MatrixXd result(x.rows(),1);
			double Iy = tanhFun(aVal,x(0,0)) + params[0] - x(1,0);
			double Ix = tanhFun(aVal,x(1,0)) - params[0] - x(0,0);
			result(0,0) = Ix;
			result(1,0) = Iy;
        ...
        //params = {g_fwd, g_cc}
        Eigen::MatrixXd oscNum(Eigen::MatrixXd x, double aVal, double params[2]){
        	int lenV = x.rows();
        	Eigen::MatrixXd result(lenV,1);
        	for(int i = 0; i<lenV; i++){
        		int fwdInd = (i-1)%lenV;
        		int ccInd = (i+lenV/2)%lenV;
        		if (fwdInd < 0){
        			fwdInd = lenV - 1;
        		}
        		double val = (tanhFun(aVal,x(fwdInd,0)) - x(i,0))*params[0] +
        				(tanhFun(aVal,x(ccInd,0)) - x(i,0))*params[1];
        		result(i,0) = val;
        	}
        	return result;
        }

        Eigen::MatrixXd fun1Der(Eigen::MatrixXd x, double aVal, double params[2]){
        	Eigen::MatrixXd result(x.rows(),1);
        	for(int i = 0; i<x.rows(); i++){
        		result(i,0) = tanhFunder(aVal,x(i,0)) - params[0];
        	}
        	return result;
        }

        Eigen::MatrixXd fun2Der(Eigen::MatrixXd x, double aVal, double params[1]){
			Eigen::MatrixXd result(x.rows(),x.rows());
			result = Eigen::MatrixXd::Constant(x.rows(),x.rows(),-1.0);
			result(0,1) = tanhFunder(aVal,x(1,0));
			result(1,0) = tanhFunder(aVal,x(0,0));
			return result;
		}

        Eigen::MatrixXd oscDer(Eigen::MatrixXd x, double aVal, double params[2]){
        	int lenV = x.rows();
        	Eigen::MatrixXd result(lenV,lenV);
        	result = Eigen::MatrixXd::Zero(lenV,lenV);
        	for(int i =0; i<lenV; i++){
        		int fwdInd = (i-1)%lenV;
				int ccInd = (i+lenV/2)%lenV;
				if (fwdInd < 0){
        		...
        		result(i,i) = -(params[0] + params[1]);
        		result(i,fwdInd) = params[0]*tanhFunder(aVal,x(fwdInd,0));
        		result(i,ccInd) = params[1]*tanhFunder(aVal,x(ccInd,0));
        	}
        	return result;
        }

        std::pair<Eigen::MatrixXd,Eigen::MatrixXd> fun1DerInterval(double aVal, double params[2], Eigen::MatrixXd bounds){
        	Eigen::MatrixXd der1 = fun1Der(bounds.col(0),aVal,params);
        	Eigen::MatrixXd der2 = fun1Der(bounds.col(1),aVal,params);
        	Eigen::MatrixXd derMin = der1.cwiseMin(der2);
        	Eigen::MatrixXd derMax = der1.cwiseMax(der2);
        	return std::make_pair(derMin,derMax);
        }

        std::pair<Eigen::MatrixXd,Eigen::MatrixXd> fun2DerInterval(double aVal, double params[2], Eigen::MatrixXd bounds){
			Eigen::MatrixXd der1 = fun2Der(bounds.col(0),aVal,params);
			Eigen::MatrixXd der2 = fun2Der(bounds.col(1),aVal,params);
			Eigen::MatrixXd derMin = der1.cwiseMin(der2);
			Eigen::MatrixXd derMax = der1.cwiseMax(der2);
			return std::make_pair(derMin,derMax);
		}

        std::pair<Eigen::MatrixXd,Eigen::MatrixXd> oscDerInterval(double aVal, double params[2], Eigen::MatrixXd bounds){
        	Eigen::MatrixXd der1 = oscDer(bounds.col(0),aVal,params);
			Eigen::MatrixXd der2 = oscDer(bounds.col(1),aVal,params);
			Eigen::MatrixXd derMin = der1.cwiseMin(der2);
			Eigen::MatrixXd derMax = der1.cwiseMax(der2);
			return std::make_pair(derMin,derMax);
        }

        std::pair<Eigen::MatrixXd,Eigen::MatrixXd> multiplyRegularMatWithIntervalMat(Eigen::MatrixXd regMat,std::pair<Eigen::MatrixXd,Eigen::MatrixXd> intervalMat){
        	Eigen::MatrixXd mat1 = regMat*intervalMat.first;
        	Eigen::MatrixXd mat2 = regMat*intervalMat.second;
        	Eigen::MatrixXd minMat = mat1.cwiseMin(mat2);
        	Eigen::MatrixXd maxMat = mat1.cwiseMax(mat2);
        	return std::make_pair(minMat,maxMat);

        }

        std::pair<Eigen::MatrixXd,Eigen::MatrixXd> subtractIntervalMatFromRegularMat(Eigen::MatrixXd regMat, std::pair<Eigen::MatrixXd,Eigen::MatrixXd> intervalMat){
        	Eigen::MatrixXd mat1 = regMat - intervalMat.first;
        	Eigen::MatrixXd mat2 = regMat - intervalMat.second;
        	Eigen::MatrixXd minMat = mat1.cwiseMin(mat2);
        	Eigen::MatrixXd maxMat = mat1.cwiseMax(mat2);
        ...
        std::pair<Eigen::VectorXd,Eigen::VectorXd> multiplyIntervalMatWithIntervalVec(std::pair<Eigen::MatrixXd,Eigen::MatrixXd> mat, std::pair<Eigen::VectorXd,Eigen::VectorXd> vec){
        	Eigen::VectorXd vec1 = mat.first*vec.first;
        	Eigen::VectorXd vec2 = mat.second*vec.first;
        	Eigen::VectorXd vec3 = mat.first*vec.second;
        	Eigen::VectorXd vec4 = mat.second*vec.second;
        	Eigen::VectorXd minVec = vec4.cwiseMin(vec3.cwiseMin(vec1.cwiseMin(vec2)));
        	Eigen::VectorXd maxVec = vec4.cwiseMax(vec3.cwiseMax(vec1.cwiseMax(vec2)));
        	return std::make_pair(minVec,maxVec);
        }

        typedef Eigen::MatrixXd (imp::*funNumDer)(Eigen::MatrixXd, double, double*);
        typedef std::pair<Eigen::MatrixXd,Eigen::MatrixXd> (imp::*funDerIntervals)(double, double*, Eigen::MatrixXd);

        std::pair<bool,Eigen::MatrixXd> checkExistenceOfSolution(double aVal, double* params, Eigen::MatrixXd hyperRectangle, funNumDer funNum, funNumDer funDer, funDerIntervals funDerInterval){
        	std::cout <<  "lower bounds " << hyperRectangle.col(0) << "\n";
        	std::cout <<  "upper bounds " << hyperRectangle.col(1) << "\n";
        	int numVolts = hyperRectangle.rows();

        	Eigen::MatrixXd startBounds(numVolts,2);
        	for(int i = 0; i<numVolts; i++){
        		startBounds(i,0) = hyperRectangle(i,0); //need to change this to incorporate multiple volts
        		startBounds(i,1) = hyperRectangle(i,1);
        	}

        	int iteration = 0;
        	while (true){
        		std::cout <<  "iteration number: " <<  iteration << "\n";
        		std::cout << "startBounds: " << startBounds << "\n";
        		Eigen::MatrixXd midPoint(numVolts,1);
        		midPoint = (startBounds.col(0) + startBounds.col(1))/2.0;
        		std::cout << "midPoint " << midPoint << "\n";
        		Eigen::MatrixXd IMidPoint = (*this.*funNum)(midPoint,aVal,params); //(numVolts,1)
        		std::cout << "IMidPoint " << IMidPoint << "\n";
        		Eigen::MatrixXd jacMidPoint = (*this.*funDer)(midPoint,aVal,params); //(numVolts,1)
        		std::cout << "jacMidPoint " << jacMidPoint << "\n";

        		Eigen::MatrixXd C = jacMidPoint.inverse();
        		Eigen::MatrixXd I = Eigen::MatrixXd::Identity(numVolts,numVolts);

        		std::pair<Eigen::MatrixXd,Eigen::MatrixXd> jacInterval= (*this.*funDerInterval)(aVal,params,startBounds);
        		Eigen::MatrixXd C_IMidPoint = C*IMidPoint;

        		std::pair<Eigen::MatrixXd,Eigen::MatrixXd> C_jacInterval = multiplyRegularMatWithIntervalMat(C,jacInterval);
        		std::pair<Eigen::MatrixXd,Eigen::MatrixXd> I_minus_C_jacInterval = subtractIntervalMatFromRegularMat(I,C_jacInterval);
        		Eigen::VectorXd vec1 = startBounds.col(0) - midPoint;
        		Eigen::VectorXd vec2 = startBounds.col(1) - midPoint;
        		std::pair<Eigen::VectorXd,Eigen::VectorXd> xi_minus_midPoint = std::make_pair(vec1,vec2);

        		std::pair<Eigen::VectorXd,Eigen::VectorXd> lastTerm = multiplyIntervalMatWithIntervalVec(I_minus_C_jacInterval, xi_minus_midPoint);

        		Eigen::MatrixXd kInterval1 = midPoint - C_IMidPoint + lastTerm.first;
        		Eigen::MatrixXd kInterval2 = midPoint - C_IMidPoint + lastTerm.second;
        		Eigen::MatrixXd kInterval(numVolts,2);
        		kInterval.col(0) = kInterval1.cwiseMin(kInterval2);
        		kInterval.col(1) = kInterval1.cwiseMax(kInterval2);

        		std::cout << "kInterval " << kInterval << "\n";

        		bool uniqueSoln = true;
        		for (int i = 0; i<numVolts; i++){
        			if (kInterval(i,0) <= startBounds(i,0) || kInterval(i,0) >= startBounds(i,1)){
        				uniqueSoln = false;
        			}
        			if (kInterval(i,1) <= startBounds(i,0) || kInterval(i,1) >= startBounds(i,1)){
        				uniqueSoln = false;
        			}
        		}
        		Eigen::MatrixXd kDiff = kInterval.col(1) - kInterval.col(0);
        		if (!uniqueSoln && kDiff.isZero(0) && (kInterval - startBounds).norm() == 0.0){
        			uniqueSoln = true;
        		}
				std::cout << "uniqueSolution " << uniqueSoln << "\n";

        		if (uniqueSoln){
        			std::cout << "Hyperrectangle with unique solution found\n";
        			std::cout <<  startBounds << "\n";
        			std::pair<bool,Eigen::MatrixXd> toReturn =  std::make_pair(true, startBounds);
        			allUniqueHyperRectangles.push_back(startBounds);
					return toReturn;
        		}

        		Eigen::MatrixXd intersect(numVolts,2);
        		for(int i = 0; i<numVolts; i++){
        			double minVal = std::max(kInterval(i,0),startBounds(i,0));
        			double maxVal = std::min(kInterval(i,1),startBounds(i,1));
        			if (minVal <= maxVal &&
        					minVal >= kInterval(i,0) && minVal >= startBounds(i,0) &&
							minVal <= kInterval(i,1) && minVal <= startBounds(i,1) &&
							maxVal >= kInterval(i,0) && maxVal >= startBounds(i,0) &&
							maxVal <= kInterval(i,1) && maxVal <= startBounds(i,1)){
        				intersect(i,0) = minVal;
        				intersect(i,1) = maxVal;
        			}
        			else{
        				intersect = Eigen::MatrixXd::Constant(numVolts,2,std::numeric_limits<double>::infinity());
        				std::cout << "hyperrectangle does not contain any solution\n";
        				std::cout << "\n";
        				return std::make_pair(false,intersect);
        			}
        		}

				Eigen::MatrixXd intervalLength = intersect.col(1) - intersect.col(0);
				Eigen::MatrixXd diffFromLast = intersect.col(1) - startBounds.col(1) + intersect.col(0) - startBounds.col(0);

				if (intervalLength.norm() < 1e-8 || (intersect - startBounds).isZero(0)){
					std::cout <<  "Found smallest hyperrectangle containing solution\n";
					std::cout << intersect << "\n";
					std::cout << "n";
					return std::make_pair(false,intersect);
        		}
        		...
        		iteration += 1;
        		//return std::make_pair(false,startBounds);
        	}
        }

        void ignoreCurrentSolution(Eigen::MatrixXd hyperRectangle, int* inValIndices){
        	int numVars = hyperRectangle.rows();
        	app* allFormulas[numVars*2];
        	int numForm = 0;

        	for(int ii = 0; ii<numVars; ii++){
        		double dist = hyperRectangle(ii,1) - hyperRectangle(ii,0);
        		int inValIndex = inValIndices[ii];
				double lowVal = hyperRectangle(ii,0);
				double highVal = hyperRectangle(ii,1);
				...
				char lowValString[64];
				snprintf(lowValString, sizeof(lowValString), "%f", lowVal);
				app* lowValApp = a.mk_real(rational(lowValString));

				char highValString[64];
				snprintf(highValString, sizeof(highValString), "%f", highVal);
				app* highValApp = a.mk_real(rational(highValString));

				app* cond1 = a.mk_ge(inVal,lowValApp);
				app* cond2 = a.mk_le(inVal,highValApp);
				//app* finalFormula = m.mk_or(m.mk_not(cond1),m.mk_not(cond2));
				//app* finalFormula = m.mk_and(cond1,cond2);
				allFormulas[numForm] = m.mk_not(cond1);
				allFormulas[numForm+1] = m.mk_not(cond2);
				numForm += 2;
				//allFormulas[ii] = finalFormula;

        	}
        	//app* theFinalFormula = m.mk_not(mk_and(m,numVars,allFormulas));
        	app* theFinalFormula = mk_or(m,numVars*2,allFormulas);

        	/*literal_buffer lits;
			unsigned num = to_app(theFinalFormula)->get_num_args();
			std::cout << "numExpr " << num << "\n";
			for (unsigned i = 0; i < num; i++) {
				expr * arg = to_app(theFinalFormula)->get_arg(i);
				ctx().internalize(arg, true);
				lits.push_back(ctx().get_literal(arg));
			}*/

			if (!ctx().b_internalized(theFinalFormula)){
				std::cout << "ASSERTING: finalFormula: " << mk_pp(theFinalFormula,m) << "\n";
				ctx().internalize_assertion(theFinalFormula, 0, 1);
				//ctx().mk_clause(lits.size(), lits.c_ptr(),0,CLS_AUX_LEMMA);
				//ctx().mark_as_relevant(theFinalFormula);
			}
        }

        void propagate() {
        	std::cout << "THEORY_LRA PROPAGATE()\n";
        	//std::cout << "m_asserted_qhead " << m_asserted_qhead << " m_asserted_atoms.size() " << m_asserted_atoms.size() << "\n";
            unsigned savedQhead = m_asserted_qhead;
        	flush_bound_axioms();
            if (!can_propagate()) {
            	//std::cout << "Returning? Remaining Case Split number: "<< ctx().getNumRemainingCaseSplits() << "\n";
                return;
            }
            //std::cout << "m_asserted_atoms in theory_lra\n";
...
#endif
                if (!m_delay_constraints) {
                	//std::cout << "in propagate " << mk_pp(ctx().bool_var2expr(bv), m) << " " << (is_true?"true":"false") << "\n";
			...
			/*int numVars = 1;
            double aVal = 1.0;
            double params[2] = {0.3,0.1};
			funNumDer funNum = &imp::fun1Num;
			funNumDer funDer = &imp::fun1Der;
			funDerIntervals funDerInterval = &imp::fun1DerInterval;
			int numOrigConstraints = 6;*/

            int numVars = 2;
            double aVal = -5.0;
            double params[1] = {0.0};
			funNumDer funNum = &imp::fun2Num;
			funNumDer funDer = &imp::fun2Der;
			funDerIntervals funDerInterval = &imp::fun2DerInterval;
			int numOrigConstraints = 16;

            /*int numVars = 4;
			double aVal = -5.0;
			double params[2] = {1.0,0.5};
			funNumDer funNum = &imp::oscNum;
			funNumDer funDer = &imp::oscDer;
			funDerIntervals funDerInterval = &imp::oscDerInterval;
			//int numOrigConstraints = 40;
			int numOrigConstraints = 12;*/

			int inIndices[numVars];
			int outIndices[numVars];
			int outIndices2[numVars];
			theory_var inValTheoryVars[numVars];
			theory_var outValTheoryVars[numVars];
			theory_var outValTheoryVars2[numVars];
			for(int tidx = 0; tidx<th.get_num_vars(); tidx++){
				expr *n = get_enode(tidx)->get_owner();
				for(int iidx = 0; iidx<numVars; iidx++){
					char inVal[80] = "x";
					char inValNum[70];
					sprintf(inValNum, "%d", iidx);
					std::strcat(inVal,inValNum);
					if (to_app(n)->get_decl()->get_name()==symbol(inVal)){
						inIndices[iidx] = get_var_index(tidx);
						inValTheoryVars[iidx] = tidx;
					}
				}
				for(int oidx = 0; oidx<numVars; oidx++){
					char outVal[80] = "y";
					char outValNum[70];
					sprintf(outValNum,"%d",oidx);
					std::strcat(outVal,outValNum);
					if (to_app(n)->get_decl()->get_name()==symbol(outVal)){
						outIndices[oidx] = get_var_index(tidx);
						outValTheoryVars[oidx] = tidx;
					}
				}

				if(numVars >= 4){
					for(int oidx2 = 0; oidx2<numVars; oidx2++){
						char outVal2[80] = "z";
						char outValNum2[70];
						sprintf(outValNum2,"%d",oidx2);
						std::strcat(outVal2,outValNum2);
						if (to_app(n)->get_decl()->get_name()==symbol(outVal2)){
							outIndices2[oidx2] = get_var_index(tidx);
							outValTheoryVars2[oidx2] = tidx;
						}
					}
				}
			}
			...
			Eigen::MatrixXd hyperRectangle(numVars,2);
			//std::cout << "ALWAYS:size of m_asserted_atoms " << m_asserted_atoms.size() << "\n";
			//std::cout << "Remaining Case Split number: "<< ctx().getNumRemainingCaseSplits() << "\n";
			//std::cout << "m_stats.m_make_feasible " << m_stats.m_make_feasible <<"\n";
			switch(lbl) {
            case l_false:
                ...
                break;
            case l_true:
            	//TODO:: add new solver stuff here
            	propagate_basic_bounds();
            	propagate_bounds_with_lp_solver();
            	if(m_asserted_atoms.size() >= numOrigConstraints){ //need to figure out a better way to check if this is a complete assignment
            		std::cout << "THEORY_LRA HERE?\n";
            		std::unordered_map<lean::var_index, rational> variable_values;
            		m_solver->get_model(variable_values);
            		std::pair<bool,Eigen::MatrixXd> firstKResult;
            		if (numVars < 4){
						double inValSolDoubles[numVars];
						double outValSolDoubles[numVars];
						Eigen::MatrixXd voltDoubles(numVars,1);
						int voltIndices[numVars];
						for(int ii = 0; ii<numVars; ii++){
							voltIndices[ii] = ii;
							lean::numeric_pair<rational> inValSol = variable_values[inIndices[ii]];
							lean::numeric_pair<rational> outValSol = variable_values[outIndices[ii]];
							lean::numeric_pair<rational> volt = variable_values[inIndices[ii]];
							...
							voltDoubles(ii,0) = (volt.x + volt.y/mpq(1000)).get_double();
							//std::cout << "volt at "<<ii<<" "<<voltDoubles(ii,0)<<"\n";
						}
						distance = 0.2;
						...
						firstKResult = checkExistenceOfSolution(aVal,params,hyperRectangle,funNum,funDer,funDerInterval);
						...
						if(false){
							...
							addTriangleConstraints(aVal, inIndices, outIndices, voltIndices,hyperRectangle);
						}
					}
            		/*else if (numVars >=4){
						double inValSolDoublesFwd[numVars];
						double inValSolDoublesCc[numVars];
						double outValSolDoublesFwd[numVars];
						double outValSolDoublesCc[numVars];
						Eigen::MatrixXd voltDoubles(numVars,1);
						int inIndicesFwd[numVars];
						int outIndicesFwd[numVars];
						int inIndicesCc[numVars];
						int outIndicesCc[numVars];
						int voltIndicesFwd[numVars];
						int voltIndicesCc[numVars];
						Eigen::MatrixXd current(numVars,1);

						for(int ii = 0; ii<numVars; ii++){
							int fwdInd = (ii-1)%numVars;
							if (fwdInd < 0){
								fwdInd = numVars - 1;
							}
							int ccInd = (ii+numVars/2)%numVars;
							voltIndicesFwd[ii] = fwdInd;
							voltIndicesCc[ii] = ccInd;
							lean::numeric_pair<rational> inValSolFwd = variable_values[inIndices[fwdInd]];
							lean::numeric_pair<rational> outValSolFwd = variable_values[outIndices[ii]];
							lean::numeric_pair<rational> inValSolCc = variable_values[inIndices[ccInd]];
							lean::numeric_pair<rational> outValSolCc = variable_values[outIndices2[ii]];
							lean::numeric_pair<rational> volt = variable_values[inIndices[ii]];
							inIndicesFwd[ii] = inIndices[fwdInd];
							outIndicesFwd[ii] = outIndices[ii];
							inIndicesCc[ii] = inIndices[ccInd];
							outIndicesCc[ii] = outIndices2[ii];
							inValSolDoublesFwd[ii] = (inValSolFwd.x + inValSolFwd.y / mpq(1000)).get_double();
							outValSolDoublesFwd[ii] = (outValSolFwd.x + outValSolFwd.y/mpq(1000)).get_double();
							inValSolDoublesCc[ii] = (inValSolCc.x + inValSolCc.y / mpq(1000)).get_double();
							outValSolDoublesCc[ii] = (outValSolCc.x + outValSolCc.y/mpq(1000)).get_double();
							voltDoubles(ii,0) = (volt.x + volt.y/mpq(1000)).get_double();
							//std::cout << "volt at "<<ii<<" "<<voltDoubles(ii,0)<<"\n";
							current(ii,0) = (tanhFun(aVal,inValSolDoublesFwd[ii]) - voltDoubles(ii,0))*params[0] + (tanhFun(aVal,inValSolDoublesCc[ii]) - voltDoubles(ii,0))*params[1];
						}
						distance = 0.2;
						Eigen::MatrixXd oneMatrix(numVars,1);
						oneMatrix = Eigen::MatrixXd::Ones(numVars,1)*distance;
						...
						if(firstKResult.first == false && !isinf(firstKResult.second(0,0))){
						//if(true){
							//hyperRectangle = oldHyper;
							...
							/*for(int ii = 0; ii<numVars; ii++){
								if (hyper1(ii,0) < 0 && hyper1(ii,1) > 0){
									if(voltDoubles(ii,0) < 0){
										hyper1(ii,1) = 0.0;
									}
									else{
								...
								if (hyper2(ii,0) < 0 && hyper2(ii,1) > 0){
									if(voltDoubles(ii,0) < 0){
										hyper2(ii,1) = 0.0;
									}
									else{
					...
					std::cout << "totalNumber of hyperrectangles found : " << allUniqueHyperRectangles.size()<<"\n";
					std::list<Eigen::MatrixXd>::const_iterator iterator;
					for(iterator = allUniqueHyperRectangles.begin(); iterator != allUniqueHyperRectangles.end(); ++iterator){
						std::cout << "hyperrectangle\n";
						std::cout << *iterator << "\n";
					}
					/*std::cout << "\n";
        ...
        void propagate_lp_solver_bound(lean::implied_bound& be) {
            ...
            lp_bounds const& bounds = m_bounds[v];
            int sizeOfBounds = bounds.size();
            bool first = true;
            for (unsigned i = 0; i < bounds.size(); ++i) {
                lp::bound* b = bounds[i];
                lp::bound boundWithoutPointer = *b;
                if (ctx().get_assignment(b->get_bv()) != l_undef) {
                    continue;
                }
                ...
                updt_unassigned_bounds(v, -1);
                TRACE("propagate_lp_solver_bound ",
