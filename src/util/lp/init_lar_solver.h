...
var_index add_term(const vector<std::pair<mpq, var_index>> & coeffs,
	...
	lar_term* newLarTerm = new lar_term(coeffs, m_v);
	vector<std::pair<mpq, unsigned>> coeffVectors = newLarTerm->coeffs_as_vector();
	for (int i = 0; i<coeffVectors.size(); i++){
		std::pair<mpq, unsigned> coeffVector = coeffVectors[i];
		//std::cout << "added\n";
	}
...
void add_var_bound_on_constraint_for_term(var_index j, lconstraint_kind kind, const mpq & right_side, constraint_index ci) {
    ...
    if (try_get_val(m_ext_vars_to_columns, j, term_j)) {
        ...
        //lar_term larTermTest = *m_orig_terms[adjusted_term_index];
